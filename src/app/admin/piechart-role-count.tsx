...
export const PieChartRoleCount = () => {
  const { data: items } = useSWRImmutable<IPieChartDate[]>('/AdminStatistic/roleCounts', fetcher);

  if (!items) {
    return;
  }

  const totalRoles = items.reduce((acc, curr) => acc + curr.value, 0);
    ...
    <div className="flex flex-col border-none gap-8">
      ...
      <div className="flex-col gap-2 text-sm">
        <div className="text-center leading-none text-muted-foreground">
          Phân tích toàn bộ vai trò của hệ thống năm nay
        </div>
      </div>
    </div>
  );
